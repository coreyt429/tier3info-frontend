<template>
  <q-page class="flex flex-start fraud-report-page">
    <div class="q-pa-md fraud-report-shell">
      <q-card class="fraud-hero text-white">
        <q-card-section class="q-pb-sm">
          <div class="text-overline fraud-hero__eyebrow">Security review</div>
          <div class="text-h4">Fraud Report</div>
        ...
        <q-card-section>
          <q-form class="row q-col-gutter-md items-end" @submit.prevent="runFraudReport">
      ...
      <q-card v-else-if="!isLoading && !statusMessage && !errorMessage" class="q-mt-md">
        <q-card-section class="text-grey-7">
          Enter an IP address or phone number to build the fraud report.
        </q-card-section>
      </q-card>
    </div>
  </q-page>
...
titleStore.setMainTitle('Fraud Report')
...
const selectedSpanLabel = computed(() => {
  return spanOptions.find((option) => option.value === span.value)?.label || span.value
})
...
function collectArrayCandidates(record) {
  const candidates = [
    record?.users,
...
function normalizeUser(user, index, defaultTimeZone) {
  ...
  const userId =
    extractNested(user, ['user_id', 'userid', 'username', 'userName', 'phone_number', 'phoneNumber']) ||
    `User ${index + 1}`
...
function normalizeResultCard(ipAddress, record, index) {
  ...
  const users = collectArrayCandidates(source).map((user, userIndex) =>
  ...
  return {
    ...
    ipAddress: String(source?.ip_address || source?.ip || ipAddress || `IP ${index + 1}`).trim(),
    organization:
      source?.organization ||
      source?.org ||
      source?.org_name ||
      source?.company ||
      source?.account_organization ||
      '',
    disposition: source?.disposition || source?.status || source?.decision || source?.rating || '',
    users,
    tone: dispositionTone(source?.disposition || source?.status || source?.decision || source?.rating || ''),
  }
}
...
async function postFraudRequest(body) {
  const endpoints = ['/locate/fraud_report', '/reports/fraud_report', '/fraud_report']
  let lastError = null
  ...
  for (const path of endpoints) {
    ...
    lastError = new Error(`No response from ${path}`)
  }

  throw lastError || new Error('Unable to locate fraud report endpoint')
}

async function runFraudReport() {
  trimInputString()
  if (!inputString.value) return

  isLoading.value = true
  statusMessage.value = `Running fraud report for "${inputString.value}"...`
  errorMessage.value = null
  reportCards.value = []

  try {
    const response = await postFraudRequest({
      input_string: inputString.value,
      span: span.value || defaultSpan,
    })
    const normalized = normalizeFraudResponse(response?.data)
    if (!normalized.length) {
      statusMessage.value = null
      errorMessage.value = 'No fraud report data was returned.'
      return
    }
    reportCards.value = normalized
    statusMessage.value = null
  } catch (error) {
    console.error('FraudReportPage: runFraudReport error:', error)
    statusMessage.value = null
    errorMessage.value = 'Unable to load the fraud report. Please try again later.'
  } finally {
...
function hydrateFromQuery(query) {
  ...
  if (querySpan && spanOptions.some((option) => option.value === querySpan)) {

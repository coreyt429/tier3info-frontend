    ...
    $(document).ready(function() {
        ...
        $(`#${table_id}`).DataTable(
            ...
            // }
            {
                ...
                scrollCollapse: true,
                buttons: [
                    'copy', {extend: 'csvHtml5',title: 'Data export'},
                    'excel', 'pdf'
                ]
            }
        );
        ...
        if(dataObject.contents.onclick){
            ...
            $(`#${table_id} tbody`).on('click', 'tr', function(){dataObject.contents.onclick(this)});
        }
...
function formatTimestamp_old(timestamp, format) {
    ...
    .replace('%B', 'MMMM')
    .replace('%p', 'A');
    console.log(`moment(${timestamp}).format(${momentFormat}) = ${moment(timestamp).format(momentFormat)}`);
...
function formatTimestamp_backup(timestamp, format, timeZone) {
    ...
    console.log(`moment(${timestamp}).tz(${timeZone}).format(${momentFormat}) = ${formattedTimestamp}`);
...
function Tier3InfoPreferences_Save_CB(data) { // deprecated
...
// FIXME: user_prefs
function Tier3InfoPreferences_Save(){ // deprecated
    ...
    .then(data => {Tier3InfoPreferences_Save_CB(data)})
    .catch(err => {console.log(err)})    
}

// FIXME: user_prefs
function TierInfoFavoriteSet(action){ // deprecated
    console.log(action);
    var request = {"mode": "ajax", "content-type": "application/json"};
    request.sessionId = get_sessionId();
...
function LoadingModal(message, animation) {
    ...
    $('body').loadingModal().off("click").on("click", function(event) {
...
function magic_menu() {
    ...
    function searchMenu(items) {
        ...
        items.forEach(item => {
            ...
            if (itemText.includes(inputTextValue) || 
    ...
    const outputHtml = `<ul class="text-white-50" style="columns:${columns}">${matches.map(match => {
...
function magic_menu_locate_callback(response){
    ...
    const outputHtml = `<ul class="text-white-50" style="columns:${columns}">${items.map(item => {
